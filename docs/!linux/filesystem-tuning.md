...
* Pour sauvegarder la table de partitions:
  ...
  1. Déterminer le nom du disque.  
     L'option -s permet de lister les disques et leur taille
  ...
  2. Sauvegarder la table de partition du disque avec l'option -d

      ``` bash
      # sfdisk -d /dev/sdb > sdb.disk
      ```

  3. Restaurer la table de partition avec l'option -f
