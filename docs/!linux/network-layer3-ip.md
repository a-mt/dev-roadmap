...
* Les switchs dirigent le traffic sur un réseau local en utilisant les adresses MAC.  
  Les *routeurs* (*router*), parfois appelés *passerelles* (*gateway*), dirigent le traffic entre les réseaux locaux et Internet en utilisant des adresses IP.

* Un routeur ordinaire connecte des segments de réseau utilisant les même protocoles.
  Une passerelle par défaut (*default gateway*), ou routeur par défaut,
  connecte des segments de réseau qui utilisent des protocoles différents et effectue une conversion entre les deux protocoles.

* Les routeurs par défaut effectuent souvent des services tels que NAT (traduction d'adresses réseau) et DHCP (founit des adresses IP aux systèmes qui ont besoin de nouvelles adresses IP). Chez vous, c'est souvent un routeur par défaut qui vous connecte au réseau de votre fournisseur d'accès à Internet.
...
* Notons qu'il existe deux types de firewalls: le firewall matériel, qui est un équipement réseau, et le firewall logiciel, qui est un programme qui tourne sur l'hôte final
...
* Il existe également des switchs capables d'opérer sur la couche 3 du modèle OSI: on parle de *layer-3 switch*.  
  Un L-3 switch peut non seulement connnecter des hôtes directement, en prenant les décisions à partir des adresses MAC pour transférer des trames Ethernet entre ses ports, mais également prendre des décisions de routage sur la base d'une table de routage et adresses IP — de couche 3. Cela permet d'assurer le routage entre deux VLAN.
...
## Du paquet à la trame

* Les communications au sein d'un réseau local se font via l'adresse MAC des appareils.

  ![](https://i.imgur.com/IEOUbOD.png)

* Pour communiquer avec un réseau distant, les paquets sont envoyés au routeur par défaut.
...
* L'*adresse IP* est utilisée pour identifier de manière un système sur le réseau: elle est donc unique par réseau.
...
* À contrario les adresses IP publiques doivent être uniques au niveau mondial — tout comme votre numéro de téléphone portable n'est utilisé par aucun autre téléphone portable dans le monde.  
...
* Bien que l'adresse MAC soit unique de par le monde, elle ne permet pas de localiser l'emplacement d'une machine: impossible de dire où se trouve le Nème exemplaire de la marque X série Y.  
...
* IPv6 utilise 8 nombres de 16 bits.  
...
* Un sous-réseau (*subnet* en anglais) consiste à diviser un réseau en différents réseaux plus petits. Le plage d'adresses d'un sous-réseau est toujours contigu
...
* Le masque de sous-réseau (*subnet mask* ou *network mask*) permet de savoir quelle partie de l'adresse IP identifie le réseau et quelle partie identifie l'hôte.

  ![](https://i.imgur.com/vZvGGFC.png)

* Techniquement, il s'agit d'un masque de bits permettant de cacher une partie de l'adresse IP: Oo peut calculer l'adresse réseau en appliquant un ET logique entre l'adresse IP et le masque de sous-réseau. Par exemple:
...
* Par exemple: un paquet de données arrive sur le réseau local d'une entreprise, avec pour destination 142.250.72.78, et le routeur exposé à Internet doit déterminer à quel sous-réseau il doit envoyer les données — routeur A, B ou C.

  La masque réseau lui permet de savoir que la partie réseau de cette adresse est 142.250.72.* et il doit donc envoyer le paquet au routeur qui gère ce segment de réseau — routeur B.

  ![](https://i.imgur.com/vLkf3NF.png)

### Classes IPv4

* Historiquement, on utilisait un système de classe comme masque:
...
* CIDR (*Classless Inter-Domain Routing*) est une méthode de masquage de réseau plus récente que les classes IPv4: à la fin d'une adresse IP, on place un `/` suivit d'un nombre. Ce nombre représente le nombre de bits impliqués dans la partie réseau de l'adresse IP. Par exemple:
  ...
  - <ins>142.250.72.78/28</ins>:  
    ...
    - Première addresse IP: 10001110.11111010.01001000.01000001 (142.250.72.65)
      Adresse de broadcast: 10001110.11111010.01001000.01001111 (142.250.72.79)  
      Dernière addresse IP: 10001110.11111010.01001000.01001110 (142.250.72.78)

* Tout comme IPv4, IPv6 utilise la notation CIDR pour représenter une plage d'adresses réseau. La partie réseau d'une adresse IPv6 est appelée le *routing* ou *prefixe*.
...
* NAT (*Network address translation*) est apparu en raison du nombre limité d'adresses disponibles avec IPv4:  
  cela permet d'avoir un réseau local avec un grand nombre machines, mais une seule adresse IP publique utilisée sur Internet.
...
* DHCP (*Dynamic Host Configuration Protocol*) est un protocole qui permet à une machine qui se connecte à un réseau d'obtenir dynamiquement sa configuration réseau — son adresse IP principalement.  
...
* Plus présicemment, pour contacter le serveur DHCP, la machine envoit une requête DHCPDISCOVER sous forme d'un broadcast sur le réseau local.  
...
* ARP (*Address Resolution Protocol*) est un protocole utilisé pour déterminer  
  ...
  1. hostA est connecté à un réseau informatique et souhaite émettre une trame Ethernet à destination d’hostB,  
     dont il connaît l’adresse IP, et qui est placé dans le même sous-réseau.
...
* La table de routage peut être construite
  ...
  - soit statiquement, avec l'administrateur qui vient configurer des routes statiques sur l'appareil
