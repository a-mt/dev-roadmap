---
title: Python
category: Web
---

* [Installer / exécuter](!python/python-install.md)
* [Introduction](!python/python-overview.md)
* [Types de données & variables](!python/python-types.md)
* [Opérateurs](!python/python-operators.md)
* [Structures de contrôle / boucles](!python/python-if.md)
* Types
  * [Nombres](!python/python-number.md)
  * [Chaînes de caractères](!python/python-string.md)
  * [Listes](!python/python-list.md)
  * [Tuples](!python/python-tuple.md)
  * [Sets](!python/python-set.md)
  * [Dictionnaires](!python/python-dict.md)
* [Fonctions](!python/python-function.md)
* [Modules](!python/python-import.md)
* [Fichiers](!python/python-file.md)
* [Regex](!python/python-regex.md)
* [Date](!python/python-date.md)
* [Exceptions](!python/python-except.md)
* [Classes & objets](!python/python-object.md)
* [Divers](!python/python-misc.md)

[Sharing data between processes](https://superfastpython.com/numpy-share-array-processes/)
